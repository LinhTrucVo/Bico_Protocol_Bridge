...
Req-DIGITALAPP-001: The component shall initialize GPIO and PWM drivers from ConfigService on startup.
   Verification: Init configures GPIO pins and PWM channels per stored config.

Req-DIGITALAPP-002: The component shall write a digital pin state (HIGH/LOW).
   Verification: WriteGpio(pin, state) sets the GPIO driver output.

Req-DIGITALAPP-003: The component shall read the current digital pin state.
   Verification: ReadGpio(pin) returns the current GPIO driver input state.

Req-DIGITALAPP-004: The component shall start PWM output on a specified channel.
   Verification: StartPwm(channel) enables PWM output at configured frequency/duty.

Req-DIGITALAPP-005: The component shall stop PWM output on a specified channel.
   Verification: StopPwm(channel) disables PWM output.

Req-DIGITALAPP-006: The component shall return INVALID_PARAM for out-of-range pin or channel IDs.
   Verification: Invalid IDs return error without hardware action.

Req-DIGITALAPP-007: The component shall return NOT_INITIALIZED if called before Init.
