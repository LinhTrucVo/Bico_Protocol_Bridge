// SpiService Implementation - Typed SPI API
...
{
    bool initialized;
} SpiService_Context_t;

static SpiService_Context_t context = {0};
...
SpiService_Status_t SpiServiceUnit_Init(void)
{
    ...
    {
        return SPISERVICE_STATUS_ERROR;
    }
...
SpiService_Status_t SpiServiceUnit_Write(uint8_t device, const uint8_t *pData, uint16_t length)
{
    if (!context.initialized)
    {
        return SPISERVICE_STATUS_NOT_INITIALIZED;
    }
    if (pData == NULL || length == 0U || length > SPI_SERVICE_CFG_MAX_TRANSFER)
    {
    ...
    {
        return SPISERVICE_STATUS_BUS_ERROR;
    }
...
SpiService_Status_t SpiServiceUnit_Transceive(uint8_t device, const uint8_t *pTxData, uint16_t txLength, uint8_t *pRxData, uint16_t rxLength)
{
    ...
    if ((txLength + rxLength) > SPI_SERVICE_CFG_MAX_TRANSFER)
    {
        return SPISERVICE_STATUS_INVALID_PARAM;
    }
