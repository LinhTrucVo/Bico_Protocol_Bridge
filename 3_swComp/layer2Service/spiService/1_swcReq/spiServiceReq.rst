...
Req-SpiService-001: The component shall initialize the SPI master driver from ConfigService on startup.
   Verification: Init retrieves SPI config and configures the SPI master driver.

Req-SpiService-002: The component shall perform SPI write (TX only) to a specified device.
   Verification: Write(dev, data, len) asserts CS, transmits data, deasserts CS.

Req-SpiService-003: The component shall perform SPI transceive (TX+RX) to a specified device.
   Verification: Transceive(dev, tx, tx_len, rx, rx_len) transmits and receives simultaneously.

Req-SpiService-004: The component shall manage CS pin assertion/deassertion for each transfer.
   Verification: CS is asserted before transfer and deasserted after (even on error).

Req-SpiService-005: The component shall return BUS_ERROR on SPI driver errors.
   Verification: Driver errors propagate as SPISERVICE_STATUS_BUS_ERROR.

Req-SpiService-006: The component shall return INVALID_PARAM for null pointers or oversized transfers.
   Verification: Invalid inputs return error without hardware action.

Req-SpiService-007: The component shall return NOT_INITIALIZED if called before Init.
